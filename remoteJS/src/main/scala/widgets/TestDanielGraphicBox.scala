...
class TestDanielGraphicBox(reload:()=>Unit,program: Box[String], ax: Box[String], maxT: Box[String], maxI: Box[String], gType: Box[String], eps: Box[String], errorBox: OutputArea)
  ...
  private def redraw(range: Option[(Double,Double)],hideCont:Boolean): Unit = try {
    ...
    (lastSyntax,lastSolver) match {
      case (syntax,Some(solver)) =>
        ...
        if (z_Title.isEmpty){
          val (markers, movingPart) = createMovingObjects2D(graph_names, "testlocalGraphic")
          js += "\n" + markers
  ...
  private def callSage() = {
    ...
    bounds = buildBounds(ax.get, maxT.get, maxI.get, gType.get, eps.get)
    val (axis, maxTime, maxIterations, graphType, perturbationUpTo) = processParsedConfig(bounds)
    RemoteBox.remoteCall("linceWS",s"G§${maxTime}§${perturbationUpTo}§${program.get}",draw)
  }
  ...
  * @return       A tuple containing the axis, max time, max iterations and graphType values.
  */
  def extractValues(config: hprog.ast.SyntaxConfig.SyntaxConfig): (List[(String, String, Option[String])],
  ...
  private def createMovingObjects2D(graphNames: List[String], divName:String): (String, String) = {
    var markers: String = ""
