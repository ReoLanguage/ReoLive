...
//////////////   
    ...
    , "PPDL - Ex.2.6c"
      ...
      -> "Example 2.6c Adaptive cruise control with an uncertain delay (given by a exponential distribution)."
    , "PPDL - Ex.2.6d (hist)"
      -> "[p,v,pl,vl]" -> "20" -> "1000" -> "histogram: p>=pl" -> "0"
      -> "// Adaptive Cruise Control (ACC)\np:=0; v:=0;  // follower  \npl:=50; vl:=10; // leader\na:=0; lambda:=8; x:=0;\nseed:=[1..50];\nwhile true {\n\tx := expn(lambda) + 1;\n  // decide to speed up (acc=2) or brake (acc=-2)\n  if (v-8)^2 + 4*(p-pl+v-9) < 0\n  then p'=v, v'= 2, pl'=vl, vl'=a for x;\n  else p'=v, v'=-2, pl'=vl, vl'=a for x;\n}"
      -> "Example 2.6d - Adaptive cruise control with an uncertain delay (given by a exponential distribution) - version that displays an histogram using 50 runs."
  ).map(x => List(  
