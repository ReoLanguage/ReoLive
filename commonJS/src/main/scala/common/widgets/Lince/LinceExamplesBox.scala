...
//////////////   
    ...
    -> "[x]" -> "6" -> "1000" -> "scatter" -> "0"
    -> "n := 6;\nx := 0; c := 0;\nwhile c<=n {\n  bernoulli (1/2)\n    x++; x--;\n\tc++;\n}\nx := x/sqrt(n);"
    -> "Example 2.1 experimenting constructs with random values."
    , "Rnd: 2.2"
      -> "[x]" -> "5" -> "1000" -> "scatter" -> "0"
      -> "x := 0;\nwhile true {\n  bernoulli (1/2)\n    x++; x--;\n  wait unif(0,1);\n}"
      -> "Example 2.2 experimenting constructs with random values."
    , "Rnd: 2.3"
      -> "[p,v]" -> "5" -> "1000" -> "scatter" -> "0"
      -> "d:=0;\np := 10 ; v := 0 ;\nwhile true {\n  d := unif (0,1) ;\n  p'=v,v'= -9.8 for d ;\n  v := -v;\n}"
      -> "Example 2.3 experimenting constructs with random values."
    , "Rnd: 2.4"
      -> "[p,v,a]" -> "5" -> "1000" -> "scatter" -> "0"
      -> "lambda:=2; d:=0;\n// the \"seed\" variable fixes\n// the pseudo-random generator\nseed:=5; \np:=0; v:=0; a:=0;\nwhile true {\n  d:=expn(lambda);\n  bernoulli (1/2)\n  \ta--; a++;\n  p'= v, v'=a for d;\n}"
      -> "Example 2.4 experimenting constructs with random values."
    , "Rnd: 2.5"
    ...
    , "Rnd: 2.6a"
      ...
      -> "Example 2.6a base for the experiments in 2.6b/2.6c with random values."
    , "Rnd: 2.6b"
      -> "[p,v,pl,vl]" -> "20" -> "1000" -> "scatter" -> "0"
      -> "// Adaptive Cruise Control (ACC)\np:=0; v:=0;  // follower  \npl:=50; vl:=10; // leader\na:=0;\nwhile true {\n\ta := unif(-1,1) ;\n  // decide to speed up (acc=2) or brake (acc=-2)\n  if (v-8)^2 + 4*(p-pl+v-9) < 0\n  then p'=v, v'= 2, pl'=vl, vl'=a for 1;\n  else p'=v, v'=-2, pl'=vl, vl'=a for 1;\n}"
      -> "Example 2.6b experimenting constructs with random values - with the wrong safety check"
    , "Rnd: 2.6c"
      -> "[p,v,pl,vl]" -> "20" -> "1000" -> "scatter" -> "0"
      -> "// Adaptive Cruise Control (ACC)\np:=0; v:=0;  // follower  \npl:=50; vl:=10; // leader\na:=0; lambda:=2; x:=0;\nwhile true {\n\tx := expn(lambda) + 1;\n  // decide to speed up (acc=2) or brake (acc=-2)\n  if (v-8)^2 + 4*(p-pl+v-9) < 0\n  then p'=v, v'= 2, pl'=vl, vl'=a for x;\n  else p'=v, v'=-2, pl'=vl, vl'=a for x;\n}"
      -> "Example 2.6c experimenting constructs with random values. - with the wrong safety check"
